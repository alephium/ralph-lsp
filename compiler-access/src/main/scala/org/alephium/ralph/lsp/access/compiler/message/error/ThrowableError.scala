...
object ThrowableError {
  ...
  /** Without title */
  def apply(throwable: Throwable, fileURI: URI): ThrowableError =
    new ThrowableError(
      title = "",
      throwable = throwable,
 ...
 */
case class ThrowableError(title: String, throwable: Throwable, index: SourceIndex) extends CompilerMessage.Error {
