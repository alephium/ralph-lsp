...
object Tree {
   ...
   */
  case class Root(statements: Seq[Statement], index: SourceIndex) extends Tree
   ...
   */
  case class Import(string: StringLiteral, path: Option[ImportPath], index: SourceIndex) extends Statement

  case class ImportPath(folder: Name, file: Name, index: SourceIndex) extends Tree

  case class Source(ast: Either[Ast.ContractWithState, Ast.Struct], index: SourceIndex) extends Statement {
   ...
   */
  case class StringLiteral(value: String, name: Name, index: SourceIndex) extends Literal

  case class Name(value: String, index: SourceIndex) extends Tree
}
