...
class PCSearcherDefinitionSpec extends AnyWordSpec with Matchers with ScalaFutures {
  ...
  "search only the strict-ast" when {
    "soft-parser is disabled" in {
      ...
      val (result, workspace) =
        createWorkspaceAndRunGoToDef(enableSoftParser = false)
  ...
  "search both strict & Soft ASTs" when {
    "soft-parser is enabled" in {
      ...
      val (result, _) =
        createWorkspaceAndRunGoToDef(enableSoftParser = true)
  ...
  private def createWorkspaceAndRunGoToDef(enableSoftParser: Boolean): (Either[CompilerMessage.Error, ArraySeq[GoToDef]], WorkspaceState.IsParsed) = {
