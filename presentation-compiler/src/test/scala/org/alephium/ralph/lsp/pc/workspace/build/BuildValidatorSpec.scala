      ...
      def doTest(config: RalphcConfig.RalphcParsedConfig) = {
        ...
        actual.value.errors should contain theSameElementsAs
          ArraySeq(
            ...
            ErrorDependencyPathIsWithinContractPath(
              SourceIndex(build.code.lastIndexOf(config.dependencyPath.value),
                          config.dependencyPath.value.length,
                          Some(build.buildURI)
                         )
            ),
            // error for field contractPath
            ErrorDependencyPathIsWithinContractPath(
              SourceIndex(build.code.lastIndexOf(config.contractPath), config.contractPath.length, Some(build.buildURI))
            )
          )
