...
object GenWorkspace {
  ...
  def genUnCompiled(
      sourceCode: Gen[List[SourceCodeState]] = Gen.listOf(GenSourceCode.genSourceCode())
  ): Gen[WorkspaceState.UnCompiled] =
    for {
      build      <- genBuildCompiled()
      sourceCode <- sourceCode
    } yield WorkspaceState.UnCompiled(
      build = build,
      sourceCode = sourceCode.to(ArraySeq)
    )

  def genParsed(
      sourceCode: Gen[List[SourceCodeState.Parsed]] = Gen.listOf(GenSourceCode.genParsed())
  ): Gen[WorkspaceState.Parsed] =
    for {
      config     <- genBuildCompiled()
      sourceCode <- sourceCode
    } yield WorkspaceState.Parsed(
      build = config,
      sourceCode = sourceCode.to(ArraySeq)
    )
  ...
  def genCompiled(
      sourceCode: Gen[List[SourceCodeState.Parsed]] = Gen.listOf(GenSourceCode.genParsed())
  ): Gen[WorkspaceState.Compiled] =
    for {
      sourceCode <- sourceCode
      parsed     <- GenWorkspace.genParsed()
    } yield WorkspaceState.Compiled(
      sourceCode = sourceCode.to(ArraySeq),
      parsed = parsed
    )
