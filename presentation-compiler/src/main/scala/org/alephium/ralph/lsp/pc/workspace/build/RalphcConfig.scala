...
object RalphcConfig {
  ...
  /** Within the context of presentation-compiler:
    ...
    *
    * [[org.alephium.ralphc.Config]] is not used during parsing because string input for fields such as `contractPath` and `artifactPath` lose the
    * original user's input String value after converting it to `Path` type, which in-case of errors displays incorrect error highlighting.
    */
  type RalphcCompiledConfig =
    org.alephium.ralphc.Config

  case class RalphcParsedConfig(compilerOptions: CompilerOptions, contractPath: String, artifactPath: String)
  ...
  /** Creates a config file.
    *
    * This can be used to generate a default config [[defaultParsedConfig]] for the user in their IDE workspace.
    *
    * @param workspacePath
    *   Workspace root path
    * @param config
