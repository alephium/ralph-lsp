...
private[search] case object HoverCodeProvider extends CodeProvider[SourceCodeState.IsParsed, (SoftAST.type, GoToDefSetting), SourceLocation.Hover] with StrictImplicitLogging {
        ...
        CodeProvider
          ...
          .flatMap {
            ...
            case SourceLocation.NodeSoft(definition, _) =>
              softAST.toNode.walkDown.find(_.data.index == definition.index) match { // Find the exact location of the definition.
                case Some(definition) =>
                  definition
                    .walkParents
                    .collectFirst { // Find the nearest parent declaration.
                      case Node(declaration: SoftAST.DeclarationAST, _) =>
                        declaration
                    }
                    .flatMap {
                      declaration =>
                        buildHover(declaration, sourceCode)
                    }
  ...
  def buildHover(
    ...
    buildHoverContent(declaration).map {
      case (content, index) =>
