...
private[sourcecode] object SourceCodeStateBuilder {
  ...
  /** Compilation results from `ralphc` do not retain the source file URIs or Path information.
    *
    * This function map the compilation results back to their respect file URIs, relying on the assumption that all type-definitions have unique
    * identifiers.
    *
    * @param parsedCode
    *   The parsed code on which this compilation was executed.
    * @param compilationResult
    *   The compilation result to process.
    * @return
    *   Workspace-level error if no source-code association was found, or else the next source-code state for each source URI.
    */
  def toSourceCodeState(parsedCode: ArraySeq[SourceCodeState.Parsed],
                        compilationResult: Either[CompilerMessage.AnyError, (Array[CompiledContract], Array[CompiledScript])]
  ): Either[CompilerMessage.AnyError, ArraySeq[SourceCodeState.CodeAware]] =
    compilationResult map { case (compiledContracts, compiledScripts) =>
  ...
  /** Maps compiled-code to it's parsed code.
    *
    * @param parsedCode
    *   Parsed source-code state used to run this compilation.
    * @param compiledContracts
    *   Resulting compiled contracts.
    * @param compiledScripts
    *   Resulting compiled scripts.
    * @return
    *   Compiled source-code state that might contain source-code level errors if a compiled source-code instance was not found its parsed state.
    */
  ...
  ): ArraySeq[SourceCodeState.CodeAware] =
    parsedCode map { sourceCodeState =>
  ...
  private def findMatchingContract(contractsToCompile: Ast.ContractWithState, compiledContracts: Array[CompiledContract]): Option[CompiledContract] =
    compiledContracts.find(_.ast.name == contractsToCompile.name)

  private def findMatchingScript(contractsToCompile: Ast.ContractWithState, compiledScripts: Array[CompiledScript]): Option[CompiledScript] =
